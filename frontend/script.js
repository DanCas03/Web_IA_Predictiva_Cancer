...
// Estado temporal de consentimiento
window.__consentState = undefined; // { save: boolean } cuando se resuelve

// Elementos del popover de consentimiento
const consentPopover = document.getElementById("consentPopover");
...
// Abrir popover cuando se intenta enviar sin consentimiento previo
form.addEventListener("submit", async (e) => {
	// Si aún no se resolvió el consentimiento, mostrar el popover y abortar envío
	if (!window.__consentState) {
		e.preventDefault();
		if (consentPopover?.showPopover) {
			consentPopover.showPopover();
		} else {
			// Fallback si no hay soporte nativo: usar confirm
			const accept = window.confirm(
				"¿Da su consentimiento para guardar los datos ingresados junto con el resultado?",
			);
			window.__consentState = { save: !!accept };
			form.requestSubmit();
		}
		return;
	}

	// Ya tenemos consentimiento resuelto; continuar con el flujo normal
	e.preventDefault();

	const saveFlag = !!window.__consentState.save;
	// Limpiar el estado para futuros envíos
	window.__consentState = undefined;
	...
	try {
		...
		// Construir URL con parámetro save si aplica
		const url = `${API_URL}/predict${saveFlag ? "?save=true" : ""}`;
...
// Eventos de los botones del popover
if (consentAcceptBtn) {
	consentAcceptBtn.addEventListener("click", () => {
		window.__consentState = { save: true };
		if (consentPopover?.hidePopover) {
...
if (consentDeclineBtn) {
	consentDeclineBtn.addEventListener("click", () => {
		window.__consentState = { save: false };
		if (consentPopover?.hidePopover) {
			consentPopover.hidePopover();
		}
		// Disparar envío del formulario
...
function validateFormData(data) {
	...
	const numericValidations = {
		...
		liver_function_score: {
			min: 0,
			max: 100,
			name: "Puntuación de función hepática",
		},
		alpha_fetoprotein_level: {
			min: 0,
			max: 200,
			name: "Nivel de alfa-fetoproteína",
		},
	};
...
function displayResults(prediction) {
	...
	actionRequiredElement.textContent =
		prediction.action_required === "immediate" ? "Inmediata" : "Preventiva";
...
function animateCircle(circleElement, percentage, color) {
	...
	circleElement.style.transition =
		"stroke-dashoffset 1.5s ease-in-out, stroke 0.5s ease";
}
...
window.addEventListener("load", async () => {
	try {
		...
		if (health.status !== "healthy") {
			showError(
				"⚠️ El servidor de predicción no está disponible. Por favor, contacte al administrador.",
			);
		}
	} catch (error) {
		console.error("Error verificando la salud del servidor:", error);
		showError(
			"⚠️ No se puede conectar con el servidor. Asegúrese de que la API esté en ejecución.",
		);
	}
});
